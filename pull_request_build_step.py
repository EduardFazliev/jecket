...
def send_file_results(file, pmd, checkstyle):
    ...
    print file_comments.send_static_check_results(pmd, checkstyle)


def static_check(file, cmd, report_flag):
    code, result = execute_linux_command(cmd)
    if code != 0:
        count = 'Error while executing static check: {}'.format(result)
    else:
        i = 0
        with open('{}_pmd.xml'.format(file), 'r') as f:
            for line in f:
...
def commit_files_handler(commit_id):
    ...
    for file in files:
        ...
        pmd_count = static_check(file, cmd, violations)
        ...
        violations = '<error'
        checkstyle_count = static_check(file, cmd, violations)
