...
def parse_args():
    ...
    parser_set_conf = subparsers.add_parser("set-conf",
                                            help="Generate conf file for jecket.")
    parser_set_conf.add_argument("-l", "--base-link", type=str,
                                 help="Base link to your BitBucket server. "
                                      "For example: http://somecompany.bitbucket.com.")
    parser_set_conf.add_argument("-u", "--username", type=str,
                                 help="Username for basic authorization"
                                      "on bitbucket server.")
    parser_set_conf.add_argument("-p", "--password", type=str,
                                 help="Password for basic authorization "
                                      "on bitbucket server.")
    ...
    parser_static_check.add_argument("-e", "--extension", type=str,
                                     help="File extension with dot.")
    ...
    static_check_group.add_argument("-f", "--file", type=str,
                                    help="Full or relative path to file, "
                                         "that will be checked.")
    static_check_group.add_argument("-p", "--pull-request",
                                    action="store_true",
                                    help="Check all files, that "
                                         "was changed in pull-request.")
    static_check_group.add_argument("-a", "--all", action="store_true",
                                    help="Check all project.")
    parser_static_check.set_defaults(func=invoke_static_check)

    # Set status of pull request.
    parser_set_status = subparsers.add_parser("set-status",
                                              help="Set status of pull-request to one of three: "
                                                   "in progress, successful or failed")
    status_group = parser_set_status.add_mutually_exclusive_group()
    status_group.add_argument("-s", "--successful", action="store_true",
                              help="Set status to SUCCESSFUL.")
    status_group.add_argument("-f", "--failed", action="store_true",
                              help="Set status to FAILED.")
    status_group.add_argument("-p", "--in-progress", action="store_true",
                              help="Set status to IN PROGRESS.")
    parser_set_status.set_defaults(func=invoke_set_status)

    parser_send_pr_comment = subparsers.add_parser('send-pr-comment',
                                                   help="Send comments to pull-request.")
    parser_send_pr_comment.add_argument("-c", "--comment", type=str,
                                        help="Comment text.")
